...
if(isset($_GET['q']) && (!empty($_GET['q']))){
	...
	$stmt = $pdo->prepare("SELECT post_id, post_title, post_slug FROM `posts` WHERE post_title LIKE ? OR post_body LIKE ? AND published=1");
...
<body>
	...
	<main class="group">
		<section class="posts-section">
			<h4>Search result for "<?php echo htmlspecialchars($_GET['q']) ?>"</h4>
			<div class="posts-snippets">	
                ...
                ?>
                        <!--Display results -->
                        <h5><?=$counter?>. <nbsp> <a href="<?php echo BASE_URL ?>posts/<?php echo $row['post_id'] ?>/<?php echo $row['post_slug'] ?>"><?php echo htmlspecialchars_decode($row['post_title']) ?> </a></h5>

...
<p>Having gone through form validation using HTML5 in a previous post, it's time we move on to form validation using JavaScript. This is still part of <em>front-end</em>  form validation, as it occurs at the client side. You should always endevour to have a multilayer form validation because when one of the layers fail, the next higher layer should be able to rise to the occasion and do a good job. In a web application, the form is the window through which users interact directly with your server, so it's important to ensure that what goes into the form is safe to be processed by your server.</p> 
<p>Validation in JavaScript is effected using <strong><em>regular expressions</em></strong> and in this tutorial we will apply the knowledge of regular expressions. Incase you are not familiar with regular expressions, you can read my tutorial on <a href="post.html.php?id=3"> Regular Expressions in JavaScript</a>. Still, in this tutorial we will try to explain everything we are going to use.</p>
...
<p class="special-p">Cross-site scripting (XSS) is a computer security vulnerability in web applications that enables an attacher to inject a client-side script into web pages viewed by others.</p>
...
<h3>Validating <code>Username</code> Field</h3>
...
<h3>Validating <code>Email</code> Field</h3>
...
<h3>Validating <code>Telephone</code> Field</h3>
...
<h3>Validating <code>Password</code> Field</h3>
...
<p>Another thing you might have noticed is that if you set focus on an input and click elsewhere, even if you did not type anything, an error alert is fired. One would expect that if nothing changed in the field there should be nothing to warrant a validation call. But the event <code>onblur</code> occurs when the input loses focus and is oblivious of whether there was a change or not.</p> 
<p>Another event listener that does the same job but behaves differently is  <code> nochange</code>. It only occurs when something changed in the input. That is what we would want. So we are going to change <code>onblur</code> to <code> onchange</code>. We will also return the <code>email</code> and <code> tel</code> type attributes to the inputs and designate the required fields. The form code should now look like the one below. </p> 
<pre class="prettyprint linenums">
&lt;form method="POST" action=""&gt;
	&lt;label for="Fullname"&gt;Full Name:&lt;/label&gt;
...
<p>We need to extend the script so that when there is an error, the error message will be displayed under the input field and at the same time the background of the field change color. We are going to use <code> document.getElementById(). innerHTML</code> DOM element selector to access and display error messages within the span tags. The <code> class="errorMessage"</code> has been styled to change the error message to red.</p> 
...
<h2>Conclusion</h2>
<p>That marks the end of front-end form validation. Once you have tested that your scripts are working well, you can add any HTML5 form validation that you might have disabled for testing purposes. HTML  and JavaScript validation rules must be consistent otherwise they may conflict and confuse users. This is just one of the steps to securing your form. You must ensure that the server-side software properly sanitizes and validates form data. </p>
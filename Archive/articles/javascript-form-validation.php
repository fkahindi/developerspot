...
<!--Validating forms using JavaScript is one of the front-end validation techniques. It uses regular expressions to validate user input before the form is leaves the browser.-->
<p>Having gone through form validation using HTML5 in a previous post, it's time we move on to form validation in JavaScript. This is still part of <em><span class="key">front-end  form validation</span></em>, as it occurs at the browser level. As a developer, you should always endevour to have a multi-layer form validation because when one of the layers fail, the next higher layer should be able to rise to the occasion and do a good job.</p> 
...
<p><span class="key">Validation in JavaScript</span> is effected using <em>regular expressions</em>. In this tutorial we are going to apply the knowledge of regular expressions to validate form data. If you are not familiar with <i><span class="key">regular expressions</span></i>, you can go through the <a href="post.html.php?id=2"> Regular Expressions in JavaScript</a> and get acquainted with these expressions. Still, in this post we will try to explain everything we are going to use.</p>
...
<p>In the form above, I have used six fields that will collect different type of user information. You will notice that I have not used the <code>required</code> <span class="key">html attribute</span> in the input fields to enable us test the effectiveness of the regular expressions. For the same reason I have used <code>type="text"</code> attribute instead of <code>type="email"</code> and <code>type="tel"</code>. The <code>id</code> attribute for each field is important, because we will use it to select the input elements in the <strong>DOM</strong> from JavaScript. Instead of <code>id</code> one can also use <code>class</code>, but <code>id</code> is ideal because it is unique in the document.</p>
...
<p>We are going to create a <code>script</code> that will take care of all the validation in the form. For simplicity, we will place the script below the page just before the <code>&lt;/body&gt;</code> closing tag. The best practice is to create an <span class="key">external script file</span> and place its reference at the same place, but that is only for scripts management purpose, it does not affect the script performance that much.</p> 
<p>The reason why JavaScripts should be placed at the bottom of the <span class="key">HTML document</span> is to allow the page to render first, before loading scripts. This improves rendering speed and especially in mobile devices that have limited bandwidth. </p> 
...
<p class="special-p"><span class="key">Cross-site scripting</span> (XSS) is a computer security vulnerability in <span class="key">web applications</span> that enables an attacker to inject a client-side script into web pages viewed by others.</p>
...
<p>Further, we would like to restrict the length of a username to be at least three but not more than fifteen characters. We will use the quantifier <code>{2,14}</code> for that. When we put all these elements together and using a <span class="key">non-capturing group</span>, we come up with the regular expression <code>/^(?:[a-zA-Z])[\w]{2,14}$/</code>, (explanation comes later). Its function then will look like the one below:</p>
<pre class="prettyprint linenums">
...
<h3><span class="key">Validating <code>Telephone</code> Field</span></h3>
...
<h3><span class="key">Validating <code>Password</code> Field</span></h3>

...
<p>In the above form, we have used six fields that will collect different type of user information. You will notice that I have not used the <code>required</code> html attribute in the input fields to enable us test the effectiveness of the regular expressions. For the same reason I have used <code>type="text"</code> attribute instead of <code>type="email"</code> and <code>type="tel"</code>. The <code>id</code> attribute for each field is important, because we will use it to select the input element from JavaScript. Instead of <code>id</code> one can also use <code>class</code>, but <code>id</code> is ideal because it is unique in the document.</p>
...
<p>You can see that this regular expression has alot of specifications to meet, as we wouldn't want a genuine email locked out. Let's build a regular expresson to meet these requirements.</p>
...
<p>When it comes to passwords, the more variety of characters users are allowed to use the stronger the passwords they can form. However, we won't allow certain characters that attackers may take advantage of and run harmful commands on our server. As far as that one goes, we can list all the characters we will accept. Normally we expect a password to consist of wordly characters: letters, numbers  or underscore. With those characters we have been using <code>/[\w]+/</code>. We want to allow other characters outside this set which we deem safe to use.</p> 
<p>But first, the quantifier, "+" we have used in the above expression does not properly set the length of the password. As it is, it allows a password with one or many characters. We need to use another quantifier to set the minimum number of characters we want to accept. Let's say 6, then we would write it as <code>/[\w]{6,}/</code>.</p> 
<p>If we also wanted to allow specific symbols, say dot ".", hyphen "-", asterisk "*" and hash "#", we can add them directly to the character set. But be careful! The hyphen "-" also signifies a range character in the set and so when it comes between certain characters such as <code>.-*</code> it can result in unexpected behaviour including stopping the whole script. To avoid that it should be placed at the extreme end or escaped. Therefore, we can add these characters in the set like this: <code>/[\w.#*-]{6,}/</code> or like this <code>/[\w.\-*#]{6,}/</code>.</p> 

<form method="post" class="reply-form" id="comment_reply_form_<?php echo $comment['comment_id']; ?>" data_id="<?php echo $comment['comment_id']; ?>">
	...
	<input type="hidden" name="oauth_provider" class="reply_form_oauth_provider" value="<?php echo $_SESSION['oauth_provider'] ?? 'direct' ?>">
	<input type="submit" id="post_reply_<?php echo $comment['comment_id']; ?>" class="post_reply" name="submit" value="Post Reply">
</form>